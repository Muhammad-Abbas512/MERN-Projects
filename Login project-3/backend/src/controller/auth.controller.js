...
//Token blackListing is done by storing the refresh token hash in the database and marking it as revoked when the user logs out. When a refresh token is presented, the server checks if the hash exists in the database and if it is not revoked. If it is revoked or does not exist, the server rejects the request, effectively blacklisting that token.
